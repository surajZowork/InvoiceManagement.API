...
export default function InvoiceForm() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      const saved = await createInvoice({ customerName, invoiceDate: date ? new Date(date).toISOString() : undefined, lines })
      setOk(`Saved invoice #${saved.id}`)
  ...
  return (
    <form onSubmit={submit} className="stack">
      ...
      <div className="stack">
        <div className="row"><strong>Line items</strong> <button type="button" onClick={addLine}>+ Line</button></div>
        {lines.map((l, i) => (
